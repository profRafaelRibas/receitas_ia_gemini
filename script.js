...
async function submitForm() {
    ...
    try {
        ...
        if (result) {
            // Se result for um array, junte os elementos
            const receita = result.join('')
            responseDiv.innerHTML = receita // Exibir diretamente
        } else if (typeof result === 'string') {
            // Se result for uma string, exiba diretamente
            responseDiv.innerHTML = result // Exibir diretamente
        } else {
            // Caso contrário, trate como erro ou exiba uma mensagem genérica
            responseDiv.innerHTML = `<p>Erro: ${result.Erro || 'Resposta inesperada do servidor'}</p>`
        }
